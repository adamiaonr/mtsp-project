classdef random_cache < cache
    %RANDOM_CACHE Implements a cache ruled by a random policy.
    ...
    properties
        ...
        % replacement probability, to be used during updateOnData()
    ...
    methods
        ...
        % class constructor
        function obj = random_cache(content_n, size)
        ...
        function [] = updateOnData(obj, inputs)
            ...
            % may include the removal of a certain cotent item. we assume 
            % that
            ...
            % 3) randomly pick elements in the CS, replace them with
            % elements in the pseudo_order vector with a probability
            % prob_rep (set to 0.5 by default).
            ...
            for i = 1:pseudo_order_n
                ...
                if (cached_n >= obj.size)
                    ...
                    if (rand > obj.prob_replacement)
                    
                        % 3.2.1) randomly pick a cached element from the CS
                        cached_idx = find(cached);
                        to_be_replaced = cached_idx(randi(length(cached_idx)));

                        % 3.2.2) replace it
                        obj.CACHE(pseudo_order(i),:) = obj.CACHE(to_be_replaced,:);            
                        obj.CACHE(to_be_replaced,:) = zeros(1, obj.size);
                ...
                % 3.4) reset the age for the newly cached Data
                obj.age(pseudo_order(i)) = 0;
                
            end
                        
            % 4) update data gathering parameters (caching time)
