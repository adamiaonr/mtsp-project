...
% 6.2.1) for each router level, gather statistics and pre-process some
% values
for i = 2:(rtr_level_n - 1)

    % 6.2.1.1) find out which routers belong to level i
    level_rtrs = find(rtr_level(:,i));

    % 6.2.1.2) simply gather the statistics for level i
    for j = 1:numel(level_rtrs)

        hit_level(:,i) = hit_level(:,i) + nodes(level_rtrs(j)).CS.stats_hits ./ (nodes(level_rtrs(j)).ifaces.stats_interests_rcvd(:, 1) + 1);    
        miss_level(:,i) = miss_level(:,i) + nodes(level_rtrs(j)).CS.stats_miss ./ (nodes(level_rtrs(j)).ifaces.stats_interests_rcvd(:, 1) + 1);

    end
    
    % 6.2.1.3) in order to get a well defined line and not simply a bunch of
...
% 6.2.2) after the statistics gathering and pre-processing, the actual
...
% 6.2.2.1) apparently, this is important
axis([0 content_n 0 1]);

xlabel('Content index');
ylabel('Hit rate');

% 6.2.2.2) some references for the dynamic plot list (both for the plot
...
% 6.2.2.3) and this is how you build up a dynamic legend...
...
% 6.2.2.4) do the sames as above, now for cache misses
subplot(1, 2, 2);
...
% levels (basically, the same procedures as those applied in 6.2 are used
% here)
...
% 6.4) same thing as 6.1, but applied to topology levels (not individual
